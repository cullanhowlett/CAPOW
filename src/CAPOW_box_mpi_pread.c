...
// Cullan's code to compute the power spectrum designed for the MockChallenge 
// simulation boxes on NERSC
...
// Hardcoded parameters for grid size and power spectrum bins
const int los=3;
const int do_interlacing=1;   // Whether or not to interlace the grids to reduce aliasing.
const int interp_order=3;     // The interpolation order for assigning data to grids.

const unsigned long long NX=512;    // size of FFT in x
const unsigned long long NY=512;    // size of FFT in y
const unsigned long long NZ=512;    // size of FFT in z

const double XMIN = 0.0;	    // Physical size of the grid
...
const unsigned long long NB = 300;      // Number of bins
const double mink = 0.0;                // minimum k to be binned
const double maxk = 1.88495559215;      // maximum k to be binned

// MPI parameters
int NTask, ThisTask;
ptrdiff_t Total_size;

// Set this parameter to do 2D binning (k, mu) of power spectrum rather than multipoles. 
// If we are doing 2D we need to know how many mu bins (nmu)
int do_2D;
const int NMU = 120;

// Set this parameter to use log-binning in k rather than linear
int do_log;
...
int main(int argc, char *argv[]) {
  ...
  int i;
  #pragma omp parallel for private(i) reduction(+:Pg0[:NB],Pg2[:NB],Pg4[:NB],Nk[:NB],Pg0_2D[:NB*NMU],Nk_2D[:NB*NMU]) collapse(3)
  for (i=Local_x_start; i<Local_x_start+Local_nx; i++) {
    for (int j=0; j<NY; j++) {
      for (int k=0; k<=NZ/2; k++) {
	
        // frequency in x
        double fx, sx = 1.0/((float)NX*dx);
        if (i <= NX/2) { 
          fx = (float)i*sx;
        } else {
